...
public class ScrollIndexCallback1 : MonoBehaviour 
{
    ...
    void ScrollCellIndex (int idx) 
    {
        ...
        index = idx;
        string objectName = "Cell " + idx.ToString();
        if (text != null) 
        {
           // print("game " + ArcadeManager.masterGamelist[idx].id);
            text.text = MoveCabsEditGamelistFilter.filteredSelectedGamelist[idx].descriptiveName;
		}
        //if (image != null)
        //{
    ...
    {
        GameObject obj = GameObject.Find("MoveCabsEditUI");
        MoveCabsEditGamelistFilter filter = obj.GetComponent<MoveCabsEditGamelistFilter>();
        filter.SetSelectedGame(index);
    }
